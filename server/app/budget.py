...
def get_budget_mode_instruction(mode: str) -> str:
    if mode == "saving":
        return """
[예산 성향: 절약 모드]
- 목표: 가용 금액을 최대한 아끼고, 예상치 못한 지출에 대비해 '미할당 여유 자금'을 가장 많이 남기는 것입니다.
- 절차: 
   1. 필수 생활비(식비, 교통비)는 일상생활에 무리가 없을 정도로만 최소한으로 방어적으로 배정하세요. 
   2. 선택 지출(쇼핑, 문화생활, 기타 등)은 극도로 보수적이고 타이트하게 배정하여 예산을 대폭 줄이세요. 
   3. ★핵심 연산 규칙★: 이 모드는 총 가용 금액의 절반 수준을 의도적으로 어떤 카테고리에도 배정하지 않고 '미할당 여유 자금'으로 남겨두는 모드입니다. 하지만 가용 금액이 너무 적을 경우 ‘미할당 여유 자금’을 마음대로 조정해도 됨
"""
    elif mode == "relaxed":
        return """
[예산 성향: 균형 모드]
- 목표: 기본 생활비와 여가비의 밸런스를 맞추되, 약간의 비상금을 남겨 안전장치를 확보하는 것입니다.
- 절차: 
  1. 최근 소비 패턴을 참고하여 기본 생활비를 적당히 유지하고, 과소비했던 카테고리는 완만하게 하향 조정하세요.
  2. 선택 지출(쇼핑, 문화생활, 기타 등)은 너무 과하지도, 너무 부족하지도 않게 배정합니다.
  3. ★핵심 연산 규칙★: 이 모드는 총 가용 금액의 일부(약 20%~30% 내외)를 비상금 및 추가 저축용 '미할당 여유 자금'으로 남겨두는 모드입니다. 하지만 가용 금액이 너무 적을 경우 ‘미할당 여유 자금’을 마음대로 조정해도 됨
"""
    else:
        return """
[예산 성향: 여유 모드]
- 목표: 최소한의 저축은 유지하면서 생활 만족도와 실천 가능성을 극대화하는 것입니다.
- 절차:
  1. 과도한 절약 압박보다는 사용자가 실제로 지킬 수 있는 현실적인 예산을 우선합니다.
  2. 식비, 문화생활, 여가성 지출(쇼핑 등)에 조금 더 유연하고 여유로운 금액을 배정하세요.
  3. ★핵심 연산 규칙★: 이 모드는 소비 만족도를 극대화하는 모드로, 총 가용 금액의 최소한(약 5%~10% 내외)만 '미할당 여유 자금'으로 남겨둡니다. 하지만 가용 금액이 너무 적을 경우 ‘미할당 여유 자금’을 마음대로 조정해도 됨
"""
