...
"""
        ...
        return """
[예산 성향: 균형 모드]
- 목표: 일상적인 삶의 질을 유지하면서도, 자산의 3분의 1을 안정적으로 저축합니다.
- 핵심 연산 규칙:
...
★핵심 연산 메커니즘★:
모든 모드에서 [미할당 여유 자금]의 비율을 먼저 지정하지 마세요. 
AI는 아래 '단계별 예산 차감 프로세스'를 엄격히 준수하여 연산한 후, [최종적으로 남은 잔액]을 할당하지 않습니다.
이 때 사용자 추가 요청을 반드시 고려해야 합니다.

1단계: 필수 생활비 선할당 (가장 높은 우선순위)
- 그 다음, 필수 생활비(식비, 교통비)를 현재 선택된 [예산 성향 모드]의 기준에 맞춰 할당하고 총액에서 차감합니다.
- 이전 달의 소비내역이 충분하다면 이걸을 기반으로 하고 소비내역이 부족하다면 한국 직장인의 소득 평균에 맞춘 예산을 추천합니다.

2단계: 선택 지출 카테고리 유연 할당 (중간 우선순위)
- 1단계 연산 후 '남은 자원'을 가지고 쇼핑, 문화, 여가 등의 카테고리 예산을 모드별 규칙에 맞게 능동적으로 책정하고 차감합니다. 
- budget_details에는 쇼핑, 문화, 여가 카테고리가 존재해야 합니다.
- 1단계 이후 남은 가용 금액을 모두 할당하는 것이 아닌 전달 또는 직장인의 평균을 고려하여 할당하고 남은 금액은 최종적으로 남은 잔액으로 간주하고 할당하지 않습니다.
- 만약 1단계를 거치고 남은 금액이 부족하다면, 선택 지출 카테고리의 예산은 AI가 판단하여 과감히 0원으로 축소해야 합니다. 절대 총 가용 금액을 초과하여 배정할 수 없습니다.
