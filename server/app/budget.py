...
4. ★중요★ budget_details에 할당된 모든 예산 금액의 총합은 절대 total_budget과 같거나 넘으면 안 되며, 반드시 여유 잔액이 남아야 합니다. 
5. ★중요★ 1, 2단계 배정 후 남은 잔액을 "기타" 등의 카테고리에 임의로 몰아주어 소진하지 마세요. 사용자의 실제 과거 지출 규모에 맞게 실용적인 금액만 배정하고 연산을 멈추어야 합니다. 잔액 처리는 시스템(3단계)이 자동으로 수행합니다.
6. JSON만 출력해라.
7. 현재 산정된 예산안이 비어 있지 않다면, 기존 예산안을 참고하여 더 현실적인 방향으로 조정해라.
8. 현재 예산안이 비어 있다면, 실제 변동 지출 기록을 기준으로 새 예산안을 만들어라.
"""
