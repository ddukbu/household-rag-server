...
- ★필수 조건★: 절약 모드 등의 이유로 선택 지출을 과감히 0원으로 축소하더라도, [당월 실시간 변동 지출 기록]에 해당 카테고리의 '이미 지출한 누적 금액'이 단 돈 1원이라도 존재한다면, 해당 카테고리의 예산은 최소한 '그 이미 지출한 금액과 동일한 금액'으로 배정해야 합니다. (예: 이미 20,000원을 썼다면 절약 모드여도 0원이 아닌 최소 20,000원을 예산으로 복구해야 마이너스가 나지 않습니다.)
