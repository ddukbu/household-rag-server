...
- 1단계 연산 후 '남은 자원'을 가지고 쇼핑, 문화, 여가 등의 카테고리 예산을 모드별 규칙 맞게 능동적으로 책정하고 차감합니다. 
- 이전 달의 소비내역 또는 직장인의 평균을 고려합니다.
- 남은 금액을 모두 할당하는 것이 아닌 여기에서 남은 금액을 추가로 미할당 여유 자금으로 포함하여 다른 카테고리에 할당하지 않습니다.
